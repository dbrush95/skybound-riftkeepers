# Skybound: Riftkeeper

Third-person gadget platformer. Name your inventor, smash crates, swing the Omniwrench, blast coral bots, and close **ten sky gates** before Captain Vesper Kael aims the Fracture.

Inspired by classic 3D action-platformers — original cast, worlds, and story. Nix talks. The left graviton boot is the expensive one.

## Play (web / 3D)

```bash
npm install
npm run dev
```

Type a captain name (default **Rook Vane**), press **Start Adventure**.

| Input | Action |
| --- | --- |
| WASD / left stick | Move |
| Space / Jump | Jump. Double jump after Bonewild. |
| J / Wrench | Melee |
| F / K / Blast | Fire current gun |
| 1–8 / Tab | Weapon wheel |
| Shift | Sprint |
| Q / E | Orbit camera |
| Esc | Pause |

Fill the gate meter (crates + bots), then walk into the Sky Gate.

## Worlds

1. Hookhaven  2. Coral Reach  3. Bonewild  4. Crash Canyons  5. Neon Docks
6. Ringfall  7. Cap Isles  8. Paintveil  9. Crystal Spire  10. Smash Deck

## Credits

After the last gate: *Ai has come a long way, but we are only at the beginning of the frontier. Keep the world moving forward. Thanks for playing. If you feel inclined, you can donate to me on X [@dallasbrush](https://x.com/dallasbrush).*
